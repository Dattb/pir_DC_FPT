...
static unsigned int tickToSleep = 0;
...
unsigned char testSUartBuff[8] = {0x55,0x55,0x55,0x55,0x55,0x55,0x55,'\n'};
unsigned char buffSendS[100] = "nguoi yeu oi co biet anh nho em nhieu lam nhung dem trong giac mo tay lam tay nghen ngao\n";
...
_attribute_ram_code_ int main (void)    //must run in ramcode
{
	...
	pirDetect(&rdPir);
	sleepBeforSend(rdPir.flag.motionFlag,0);
	user_init();
    irq_enable();
	#if (MESH_USER_DEFINE_MODE == MESH_IRONMAN_MENLO_ENABLE)
	LOG_USER_MSG_INFO(0, 0,"[mesh] Start from SIG Mesh", 0);
	#endif
//	unsigned char pir_send_buff[2] = {0x0a,0x01};
//	mesh_tx_cmd2normal(0xe5,(u8 *) pir_send_buff,2,0x0025,0x0001,2);
	if(is_provision_success()){
		softUartSend(&sUart1,"rdPir.flag.motionFlag:");
		softUartSendInt(rdPir.flag.motionFlag);
	}
	sensorSendData(rdPir.flag.motionFlag,0);
	//led_init();
	//ledTestInit();
	tickToSleep = clock_time_s();
	...
	while (1) {
		...
		#endif
		main_loop();
		if( clock_time_ms() - tickToSleep >= 500){
			sleepAfterSend();
		}
